...
void setup() {
  pixelDensity(1);
  size(400, 400, P2D);// Animation + Size of the canvas
  
  //Setting up the variables to begin with
  y = height / 2; //also equal to 400/2 or 200
  vy = 0;
  gravity = 1;
  squish = 1;
  counter = 0;
...
void draw () {
  background(200, 230, 180); //the light green that covers the background and removes the previous drawing of the blob
  //Physics!
...
void drawBlob(float x, float y, float squish) { //defining my own function of drawing blob (slime blob)
  ...
  ellipse(x - blobWidth * 0.15, y - blobHeight * 0.2, blobWidth * 0.35, blobHeight * 0.25); // on the top left
  fill(100,160,80,80);
  noStroke();
  ellipse(x, 310, blobWidth * 0.9, 14);
  ...
  fill(102,0,0,200);
  textSize(20);
  text("X", x + blobWidth * 0.20, y - blobHeight * 0.06);
  
  // Mouth [happy smile :)]
  stroke(30,100,40);
  strokeWeight(3.5);
  noFill();
  arc(x, y + blobHeight * 0.15, 28, 16, 0, PI); //the curve of the smile on the scary slime
  noStroke();
}
